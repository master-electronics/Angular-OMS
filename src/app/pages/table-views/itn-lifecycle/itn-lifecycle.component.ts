...
import { FetchItnLifecycleGQL, FindItnTemplateGQL, FindItnColumnsGQL } from 'src/app/graphql/tableViews.graphql-gen';
        ...
        map((res) => {
          ...
          this.tableData = res.data.fetchITNLifecycle.map((item) => {
            ...
            if (item.PartNumber && item.ProductCode) {
              result["PartCode"] = item.PartNumber.trim().toUpperCase()+"-"+item.ProductCode.trim().toUpperCase();
            }
      ...
      this._fetchDrilldown
        ...
        .subscribe((res) => {
          if (res.data.fetchITNLifecycleDrillDown.length > 0) {
            for (let i = 0; i < res.data.fetchITNLifecycleDrillDown.length; i++) {
              ...
              this.drilldownTableData2.push(
                {
                  ...
                  DistributionCenter: (row.DistributionCenter)?row.DistributionCenter:"",
                  Quantity: (row.Quantity)?row.Quantity:null,
                  ShipmentMethod: (row.ShipmentMethod)?row.ShipmentMethod:"",
                  ShipmentMethodDescription: (row.ShipmentMethodDescription)?row.ShipmentMethodDescription:"",
                }
              );
            }
          }
