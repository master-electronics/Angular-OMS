...
export class ValueMapping implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.sourceEntityListSubscription.add(
      this.fetchEntityList
        .fetch({ type: 'S' }, { fetchPolicy: 'network-only' })
        .subscribe(
          (res) => {
    ...
    this.targetEntityListSubscription.add(
      this.fetchEntityList
        .fetch({ type: 'T' }, { fetchPolicy: 'network-only' })
        .subscribe(
          (res) => {
            res.data.fetchEntityList.map((item) => {
                ...
                (element) => {
                  this.editCache[element._id.toString()] = {
                    ...
                    data: {
                      ID: element._id,
                      SourceData:
                        element.SourceSystemName +
                        ':' +
                        element.SourceTableName +
                        ':' +
                        element.SourceColumnName,
                      TargetLocation:
                        element.TargetSystemName +
                        ':' +
                        element.TargetTableName +
                        ':' +
                        element.TargetColumnName,
  ...
  saveEdit(id: number): void {
    ...
    const targetParts: string[] =
      this.editCache[id].data.TargetLocation.split(':');
      ...
      this.updateValueMap
        ...
        .subscribe(
          () => {
            ...
            this.viewData[index].SourceData =
              this.editCache[id].data.SourceData;
            this.viewData[index].TargetLocation =
              this.editCache[id].data.TargetLocation;
            this.viewData[index].SourceValue =
              this.editCache[id].data.SourceValue;
            this.viewData[index].TargetValue =
              this.editCache[id].data.TargetValue;
  ...
  addValueMap(): void {
    ...
    if (
      ...
      /\S/.test(this.newMappingSourceValue) &&
      this.newMappingTargetValue &&
      /\S/.test(this.newMappingTargetValue)
    ) {
