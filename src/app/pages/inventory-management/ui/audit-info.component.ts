  ...
  template: `
    <div *ngIf="auditInfo">
      <div nz-row [nzGutter]="{ xs: 8, sm: 16, md: 24, lg: 32 }">
        <div
          *ngIf="auditInfo.Inventory?.Product?.MICPartNumber"
          nz-col
          nzSpan="10"
        >
          <img
        ...
        <div nz-col nzSpan="14" class="text-black md:text-lg lg:text-xl">
          ...
          <div *ngIf="auditInfo.Inventory?.Product?.PartNumber">
            <span class="mr-2 font-medium">Part:</span>
            <span class="justify-self-start text-blue-600">{{
              auditInfo.Inventory.Product.PartNumber
            }}</span>
          </div>
          <div *ngIf="auditInfo.Inventory?.Product?.PackType">
            <span class="mr-2 font-medium">Pack Type:</span>
            <span class="justify-self-start text-blue-600">{{
              auditInfo.Inventory.Product.PackType
            }}</span>
          </div>
          <div *ngIf="auditInfo.Inventory?.Product?.PackQty">
            <span class="mr-2 font-medium">Pack Qty:</span>
            <span class="justify-self-start text-blue-600">{{
              auditInfo.Inventory.Product.PackQty
            }}</span>
          </div>
          <div *ngIf="auditInfo.Inventory?.Product?.UOM">
            <span class="mr-2 font-medium">UOM:</span>
            <span class="jusify-self-start text-blue-600">{{
